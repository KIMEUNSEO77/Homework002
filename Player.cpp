...
void CPlayer::Rotate(float x, float y, float z)
{
	...
	{
		/*로컬 x-축을 중심으로 회전하는 것은 고개를 앞뒤로 숙이는 동작에 해당. 그러므로 x-축을 중심으로 회전하는
		각도는 -89.0~+89.0도 사이로 제한. x는 현재의 m_fPitch에서 실제 회전하는 각도이므로 x만큼 회전한 다음
		Pitch가 +89도 보다 크거나 -89도 보다 작으면 m_fPitch가 +89도 또는 -89도가 되도록 회전각도(x)를 수정.*/
		if (x != 0.0f)
		{
			m_fPitch += x;
		...
		// 카메라를 x, y, z 만큼 회전. 플레이어를 회전하면 카메라가 회전하게 된다.
...
CCamera* CAirplanePlayer::ChangeCamera(DWORD nNewCameraMode, float fTimeElapsed)
{
	...
	{
	case FIRST_PERSON_CAMERA:
		// 플레이어의 특성을 1인칭 카메라 모드에 맞게 변경. 중력은 적용하지 않는다.
		SetFriction(200.0f);
	...
	case SPACESHIP_CAMERA:
		// 플레이어의 특성을 스페이스-쉽 카메라 모드에 맞게 변경. 중력은 적용하지 않는다.
		SetFriction(125.0f);
		SetGravity(XMFLOAT3(0.0f, 0.0f, 0.0f));
	...
	case THIRD_PERSON_CAMERA:
		...
		// 3인칭 카메라의 지연 효과를 설정. 값을 0.25f 대신에 0.0f와 1.0f로 설정한 결과를 비교하기 바란다.
