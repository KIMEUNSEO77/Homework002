...
protected:
	...
	// 카메라 로컬 x축(Right), y축(Up), z축(Look) 벡터
	XMFLOAT3 m_xmf3Right;
	XMFLOAT3 m_xmf3Up;
	XMFLOAT3 m_xmf3Look;

	// 카메라 x축, z축, y축으로 얼만큼 회전했는지 나타내는 각도
	float m_fPitch;  // x축 회전
	float m_fRoll;   // z축 회전
	float m_fYaw;    // y축 회전

	// 카메라 종류 (1인칭 카메라, 스페이스쉽 카메라, 3인칭 캄라)
	DWORD m_nMode;

	// 플레이어가 바라볼 위치 벡터, 주로 3인칭 카메라에서 사용
	XMFLOAT3 m_xmf3LookAtWorld;
	// 플레이어와 카메라의 오프셋 나타내는 벡터, 주로 3인칭 카메라에서 사용
	XMFLOAT3 m_xmf3Offset;
	// 플레이어가 회전할 때 얼만큼 시간을 지연시킨 후 카메라를 회전시킬 것인가
	float m_fTimeLag;

	// 카메라의 뷰 변환 행렬과 투영 변환 행렬
...
// 카메라 이동과 회전이 플레이어 객체의 이동, 회전과 완전히 일치
